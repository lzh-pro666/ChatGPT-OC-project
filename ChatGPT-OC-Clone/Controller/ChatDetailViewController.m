...
- (void)typeNextCharacter {
    ...
    // --- 优化布局更新 --- 
    BOOL isFinalCharacter = (self.typingBuffer.length == 0);
    BOOL containsNewline = [nextChar isEqualToString:@"\n"];
    static NSInteger charCountSinceLastUpdate = 0;
    charCountSinceLastUpdate++;
    
    // 只在遇到换行符、打印完成 或 累积一定字符数后才执行tableView更新
    BOOL needsTableViewUpdate = containsNewline || isFinalCharacter || (charCountSinceLastUpdate >= 100); // 每100个字符更新一次

    BOOL wasScrolledToBottom = [self isTableViewScrolledToBottom]; // 记录更新前是否在底部
    ...
    if (needsTableViewUpdate) {
        ...
        charCountSinceLastUpdate = 0; // 重置计数器
    }
    ...
    if (wasScrolledToBottom) {
        // 轻微延迟滚动，等待可能的布局更新完成
        dispatch_async(dispatch_get_main_queue(), ^{
             [self scrollToBottomImmediate]; // 使用无动画滚动减少卡顿感
        });
    }
    
    // 如果缓冲区为空，且当前响应已完成，则保存数据
    if (isFinalCharacter && !self.currentStreamingTask) {
        // 保存最终的消息内容
